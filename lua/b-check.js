...
function pick_nonce() {
    var quality = new Uint8Array(18);
    var wc = window.crypto || window.msCrypto;
    if(wc) {
        wc.getRandomValues(quality);
    } else {
        die_now("We need crypto.getRandomValues feature.");
    }

    var s = String.fromCharCode.apply(null, quality);
	s = btoa(s).replace(/[=,+\/]/g,'z')
